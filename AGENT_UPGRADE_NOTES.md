...
- **P2 follow-ups if the trial works:** PPTX export step reusing `write_pptx_from_template` (mirror `fdd_utils/pptx.py`); a Chinese-output variant (the reference `prompts.yml` has full Chi sections to distill); an HR-databook sibling skill (`python-pptx-hr` has the same shape).
