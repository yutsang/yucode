...
The first time yucode runs, it seeds `~/.yucode/settings.yml` (the live, editable config `yucode show-config`/`config-path` operate on) from the bundled template at `coding_agent/config.yml` beside the package source. It contains a `provider:` block, runtime options, tool filters, and MCP server entries.
...
The agent uses an OpenAI-compatible chat completions API. Configure the single `provider:` block in your config:
...
provider:
  ...
  api_key: "sk-..."
  model: "gpt-4o"
  chat_path: /chat/completions        # appended to base_url unless append_chat_path: false
```

Azure-style gateways (deployment-per-model URLs, `api-key` header auth) set `api_version` instead of `chat_path` — see `provider.api_version` in `config/settings.py`. `omit_params` drops body fields a given model/gateway rejects (e.g. `[temperature]` for reasoning models that only accept their default); the provider also self-heals on a 400 that names the offending param. Override the model at runtime with `--model`.
...
The core agent's only hard dependencies are `prompt_toolkit` (interactive REPL --
falls back to plain `input()` if missing) and, on Python <3.11, `tomli` (stdlib
`tomllib` covers 3.11+). Everything else is the standard library. Optional
extras add specific built-in tools (not MCP servers -- these ship directly in
`tools/office.py`/`tools/web.py`, no separate process):

| Extra      | Install command                      | What it enables                          |
|------------|---------------------------------------|-------------------------------------------|
| `server`   | `pip install yucode-agent[server]`    | HTTP + SSE server (`aiohttp`)             |
| `excel`    | `pip install yucode-agent[excel]`     | Excel tools (`openpyxl`)                  |
| `word`     | `pip install yucode-agent[word]`      | Word tools (`python-docx`)                |
| `pptx`     | `pip install yucode-agent[pptx]`      | PowerPoint tools (`python-pptx`)           |
| `pdf`      | `pip install yucode-agent[pdf]`       | PDF tools (`pdfplumber`)                  |
| `finance`  | `pip install yucode-agent[finance]`   | Finance tools (`yfinance`, `pandas`)      |
| `office`   | `pip install yucode-agent[office]`    | excel + word + pptx + pdf together        |
| `all`      | `pip install yucode-agent[all]`       | Everything above                          |

Each office/PDF tool fails with a clear "install X" message (not a traceback)
if its extra isn't installed, naming the exact `pip install` to run.
