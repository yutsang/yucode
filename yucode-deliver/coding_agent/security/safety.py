...
# A bare drive root (C:\, C:, C:\*), %USERPROFILE%, $env:USERPROFILE, or ~ —
# the Windows equivalents of "/" and "~" in the Unix rm -rf patterns below.
_WIN_ROOT_TARGET = r"(?:[a-zA-Z]:(?:[\\/]{0,2})\*?|%USERPROFILE%\\?|\$env:USERPROFILE\\?|~\\?)"
...
_DANGEROUS_FS_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    # --- Windows / cmd.exe / PowerShell equivalents -----------------------
    # The bash tool runs through cmd.exe on Windows (tools/shell.py, shell=True)
    # and there's no OS sandbox there (bwrap is Linux-only) — these patterns
    # give the same "catastrophic only" accident-prevention parity the Unix
    # patterns above give. Not a malware filter; just stops the model from
    # fat-fingering a whole-drive or whole-home wipe.
    (
        re.compile(rf"\bdel\b(?=.*\s/s\b)(?=.*{_WIN_ROOT_TARGET}(?:\s|$))", re.I),
        "del /s targeting a drive root or user profile would destroy files",
    ),
    (
        re.compile(rf"\b(?:rd|rmdir)\b(?=.*\s/s\b)(?=.*{_WIN_ROOT_TARGET}(?:\s|$))", re.I),
        "rd /s targeting a drive root or user profile would destroy files",
    ),
    (re.compile(r"\bformat\s+[a-zA-Z]:", re.I), "format would erase a Windows drive"),
    (
        re.compile(rf"\bRemove-Item\b(?=.*-Recurse\b)(?=.*-Force\b)(?=.*{_WIN_ROOT_TARGET}(?:\s|$))", re.I),
        "Remove-Item -Recurse -Force on a drive root or home directory would destroy files",
    ),
    (
        re.compile(r"\breg\s+delete\s+(?:HKLM|HKCU|HKEY_LOCAL_MACHINE|HKEY_CURRENT_USER)(?:\s+/f)?\s*$", re.I),
        "reg delete on a root registry hive would corrupt Windows",
    ),
    (re.compile(r"\bdiskpart\b", re.I), "diskpart can repartition or wipe drives"),
]
