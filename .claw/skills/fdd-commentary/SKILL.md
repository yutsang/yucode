...
3. **Extract and cross-check — mandatory, per account, before drafting a single sentence.**
   - The `Financials` sheet (some databooks split this per entity/branch, e.g. `Financials - NB`/`Financials - HN` — treat each as its own Financials sheet if so) holds the full Balance Sheet and Income Statement; other sheets are per-account detail (sheet name ≈ account name).
   - **Call `locate_databook_stage_columns` on the Financials sheet before reading any figures.** Real databooks lay out the same dates 2-5 times side by side under different bases (`Mgt acc`, `Audited`, `Indicative adjusted`, and sometimes `Audit adjustment`/`Indicative adjustment` delta columns) with only a text label distinguishing the groups — no merged cells or other structural hint. Visually picking the right block of a 10-30+ column sheet is exactly the kind of thing that silently goes wrong; this tool finds the basis-label row deterministically and returns each canonical stage's column indices + dates. **Use ONLY the columns returned under `"Indicative adjusted"`.** Ignore `Mgt acc`, `Audited`, `Audit adjustment`, and `Indicative adjustment` (that last one is the period's adjustment *delta*, not the adjusted balance — do not substitute it) entirely. If the tool returns `found: false` (no recognizable basis row), fall back to reading the header rows directly and look for the same label (English "Indicative adjusted" or Chinese 示意性調整後/示意性调整后, occasionally written 示意性调整数 by some clients) — same rule, just without the tool's help.
   - Unit marker `CNY'000` means raw values are thousands of CNY — multiply by 1,000 before formatting. The tool's `unit_marker` field reports this per sheet; re-check it per sheet regardless.
   - The latest period is the rightmost Indicative-adjusted column (per the tool's `date` field per column); that is the period a BS bullet opens with.
