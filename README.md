...
`pip uninstall yucode-agent` removes it along with the package. `python -m coding_agent.interface.cli`
too long to type every time? `yucode.bat` at the repo root is a plain-text wrapper
(not generated by pip, just checked into the repo) that forwards `yucode <command>`
to the same invocation — run it from this folder exactly like the real command.
