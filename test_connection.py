...
Usage examples:
  ...
  3. KPMG Workbench gateway (Azure-OpenAI-compatible), the go/no-go check
     for a company-PC deployment — see WORKBENCH_PLAN.md section 7:
     python test_connection.py --azure --base-url https://api.workbench.kpmg/genai/azure/openai \\
         --api-key <subscription-key> --model gpt-5-5-2026-04-24-gs-sdc --charge-code 0000
     python test_connection.py --azure --test-tools ...   # + function-calling probe (critical: agent needs this)
...
USER_EDITABLE_SETTINGS = {
    ...
    "all_auth": True,
    "run_sdk_probe": True,
    # KPMG Workbench (--azure mode) settings — see WORKBENCH_PLAN.md section 2.
    "azure": False,
    "api_version": "2024-12-01-preview",
    "subscription_key": "",   # blank = reuse api_key (they're the same value in practice)
    "charge_code": "0000",
...
def _azure_url(base_url: str, model: str, api_version: str) -> str:
    """Build the Azure-OpenAI-style deployment URL the Workbench gateway expects.

    Mirrors coding_agent/core/providers.py::OpenAICompatibleProvider._build_url
    when provider.api_version is set — kept as a literal duplicate here (not
    an import) so this script stays runnable before yucode itself is installed.
    """
    return f"{_normalize_base_url(base_url)}/deployments/{model}/chat/completions?api-version={api_version}"


def _azure_headers(
    *, api_key: str, subscription_key: str, charge_code: str, region_override: str, stream: bool,
) -> dict[str, str]:
    return {
        "Content-Type": "application/json",
        "Accept": "text/event-stream" if stream else "application/json",
        "api-key": api_key,
        "Ocp-Apim-Subscription-Key": subscription_key or api_key,
        "x-kpmg-charge-code": charge_code,
        "x-kpmg-region-override": region_override,
    }


# Minimal, deliberately trivial function-calling schema — the goal is only to
# see whether the gateway honours `tools` + `tool_choice` at all, not to
# exercise anything yucode's real tools do.
_PROBE_TOOL = {
    "type": "function",
    "function": {
        "name": "get_weather",
...
def _run_azure_attempt(
    attempt: AzureAttempt,
    *,
    url: str,
    api_key: str,
    ...
    headers = _azure_headers(
        api_key=api_key, subscription_key=subscription_key,
        charge_code=charge_code, region_override=region_override,
        stream=attempt.stream,
    )
    payload: dict[str, Any] = {
        ...
        "messages": [{"role": "user", "content": message}],
        "stream": attempt.stream,
        # Deliberately no 'temperature' — GPT-5-class models on this gateway
        # reject anything but their default (see WORKBENCH_PLAN.md section 2).
    }
    if attempt.with_tools:
...
def _print_azure_result(label: str, url: str, result: dict[str, Any]) -> None:
    status = "OK" if result.get("ok") else "FAIL"
    print(f"[AZURE] {status}  {label}")
    print(f"        url:         {url}")
    print(f"        http_status: {result.get('http_status')}")
    if result.get("text"):
        print(f"        text:        {result['text'][:120]!r}")
    if "tool_calls" in result:
        note = " <-- function calling WORKS" if result.get("tool_calls") else " <-- NO tool_calls in response"
        print(f"        tool_calls:  {result.get('tool_calls')}{note}")
    if "chunk_count" in result:
        note = " <-- streaming WORKS" if result.get("chunk_count") else " <-- no SSE chunks received"
        print(f"        chunk_count: {result.get('chunk_count')}{note}")
    if result.get("error_type"):
        print(f"        error_type:  {result['error_type']}")
    if result.get("error"):
        print(f"        error:       {result['error']}")
    if result.get("body_preview"):
        print(f"        preview:     {result['body_preview'][:200]!r}")
    print()


def _run_azure_probe(args: argparse.Namespace) -> int:
    """KPMG Workbench go/no-go check — see WORKBENCH_PLAN.md sections 2, 4 (WI-4), 7.

    Unlike the generic fuzzing matrix in main(), this mode already knows the
    correct URL/header/body shape (verified against a working production
    client) and just confirms this specific gateway/key/model combination
    actually works before the on-site checklist moves on to `yucode chat`.
    """
    url = _azure_url(args.base_url, args.model, args.api_version)
    subscription_key = args.subscription_key or args.api_key
    # include_insecure doubles as "use verify_tls=False" here — corporate TLS
    # interception is a verified fact of this gateway (WORKBENCH_PLAN.md
    # section 2), not something worth fuzzing both ways by default.
    verify_tls = not args.include_insecure

    print("KPMG Workbench (Azure-OpenAI-compatible) probe")
    print("================================================\n")
    print(f"url:             {url}")
    print(f"model:           {args.model}")
    print(f"charge_code:     {args.charge_code}")
    print(f"region_override: {args.region_override}")
    print(f"verify_tls:      {verify_tls}")
    print(f"test_tools:      {args.test_tools}")
    ...
    plain = _run(AzureAttempt(label="plain completion (no tools, no stream)", stream=False, with_tools=False, verify_tls=verify_tls))
    connectivity_ok = bool(plain.get("ok")) and bool(plain.get("text"))
    ...
    if args.include_stream:
        stream_result = _run(AzureAttempt(label="streaming (SSE)", stream=True, with_tools=False, verify_tls=verify_tls))
        streaming_ok = bool(stream_result.get("ok")) and int(stream_result.get("chunk_count") or 0) >= 1

    print("Summary")
    print("-------")
    print(f"{'PASS' if connectivity_ok else 'FAIL'}  connectivity (plain completion returns content)")
    if tools_ok is not None:
        print(f"{'PASS' if tools_ok else 'FAIL'}  function calling (tool_calls >= 1) -- REQUIRED, the agent cannot work without this")
    if streaming_ok is not None:
        label = "PASS" if streaming_ok else "INFO"
        print(f"{label}  streaming (SSE chunks received) -- optional; ship with provider.streaming_mode: no_stream if this fails")

    hard_fail = not connectivity_ok or (tools_ok is False)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="KPMG Workbench (Azure-OpenAI-compatible) go/no-go probe instead of the generic fuzzing matrix. See WORKBENCH_PLAN.md.",
    )
    parser.add_argument("--api-version", default=USER_EDITABLE_SETTINGS["api_version"], help="--azure: api-version query param.")
    parser.add_argument(
        "--subscription-key", default=USER_EDITABLE_SETTINGS["subscription_key"],
        help="--azure: Ocp-Apim-Subscription-Key header value; blank reuses --api-key.",
    )
    parser.add_argument("--charge-code", default=USER_EDITABLE_SETTINGS["charge_code"], help="--azure: x-kpmg-charge-code header value.")
    parser.add_argument("--region-override", default=USER_EDITABLE_SETTINGS["region_override"], help="--azure: x-kpmg-region-override header value.")
    parser.add_argument(
        "--test-tools", action="store_true",
        help="--azure: also probe function-calling support (tools + tool_choice) -- the agent cannot work without this.",
    )
    parser.set_defaults(
