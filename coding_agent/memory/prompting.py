...
def _doing_tasks_section(dedup_threshold: int = 3) -> str:
    ...
    return "\n".join(
        [
            ...
            "1. SPLIT the query into individual keywords first (ignore typos, spaces, punctuation).",
            "   Example: 'queueing theory basics' → keywords: ['queueing', 'theory', 'basics']",
            "2. Search filenames: glob_search '**/*keyword*' for EACH keyword separately.",
            "   A hyphenated file like 'queueing-theory.md' only matches if you search for 'queueing'.",
            "3. Search content: grep_search each keyword separately (not the whole phrase).",
            "   If grep_search returns a 'partial_matches' JSON key, open those files directly —",
            "   that means exact match failed (likely a typo) but the listed files are relevant.",
            "4. If a catalog or index exists (wiki/index.md, README), read it first.",
